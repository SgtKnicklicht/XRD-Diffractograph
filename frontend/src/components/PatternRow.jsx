...
export default function PatternRow({ pattern, index, onChange, onRemove, onNormalize, onToggleReference, hasMeasurement = false, dragActive = false, onDragStart, onDragOver, onDrop }) {
    ...
    return <div
        ...
        onDrop={onDrop}
        className={`surface fade-up p-2.5 space-y-2 transition-all ${dragActive ? "ring-1 ring-[var(--amber)]" : ""}`}
        style={{ borderLeft: `3px solid ${pattern.color}`, opacity: pattern.visible ? 1 : 0.6 }}
    >
        <div className="flex items-center gap-2">
            <span className="text-[var(--ink-3)] cursor-grab active:cursor-grabbing" title="Drag to reorder"><GripVertical size={14} /></span>
            <span className="mono text-[9px] text-[var(--ink-3)] w-4 text-right">{index}</span>
            <button onClick={() => update({ collapsed: !collapsed })} className="text-[var(--ink-3)] hover:text-[var(--ink-0)]">{collapsed ? <ChevronRight size={15} /> : <ChevronDown size={15} />}</button>
            <button data-testid={`toggle-visible-${pattern.id}`} onClick={() => update({ visible: !pattern.visible })} className="text-[var(--ink-2)] hover:text-[var(--ink-0)] transition-colors">{pattern.visible ? <Eye size={15} /> : <EyeOff size={15} />}</button>
        ...
        {!collapsed && <>
            <button data-testid={`reference-toggle-${pattern.id}`} onClick={onToggleReference} className={`w-full flex items-center justify-between gap-2 px-2 py-1.5 rounded-md border transition-all ${pattern.isReference ? "border-[var(--teal)] bg-[rgba(77,217,200,0.08)] text-[var(--teal)]" : "border-[var(--line)] text-[var(--ink-2)] hover:text-[var(--ink-0)] hover:border-[var(--ink-3)]"}`}><span className="flex items-center gap-1.5 mono text-[10px]">{pattern.isReference ? <CheckCircle2 size={12} /> : <Circle size={12} />}reference pattern</span><span className="mono text-[10px] opacity-70">{FORMAT_LABEL[pattern.source_format] || pattern.source_format}</span></button>
            {pattern.isReference && hasMeasurement && onNormalize && <button data-testid={`normalize-${pattern.id}`} onClick={onNormalize} className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-md text-[10px] mono text-[var(--ink-2)] hover:text-[var(--ink-0)] bg-[var(--bg-2)] hover:bg-[var(--bg-3)] border border-[var(--line)] transition-all"><Scaling size={11} />normalize to measurement</button>}
