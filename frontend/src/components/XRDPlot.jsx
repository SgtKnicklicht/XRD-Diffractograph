...
function getVisibleYBounds(patterns) {
    let min = Infinity;
    let max = -Infinity;
    for (const p of patterns) {
        ...
        for (const v of ys) {
            const shown = v * p.scale + p.offset;
    ...
    return { min: min - span * 0.04, max: max + span * 0.08 };
}

function buildTraces(patterns, yBaseline) {
    const traces = [];
    for (const p of patterns) {
        if (!p.visible) continue;
        const yShown = (p.processed?.y ?? p.y).map((v) => v * p.scale + p.offset);
        if (p.mode === "droplines" || p.isReference) {
            const xs = [];
            const ys = [];
...
const AXIS_TITLES = { x: "2θ [°]", y: "Rel. Intensity [a.u.]" };
const COMMON_LAYOUT = { margin: { l: 82, r: 30, t: 30, b: 64 }, dragmode: "zoom" };
const THEME_LAYOUTS = {
    dark: { paper_bgcolor: "rgba(0,0,0,0)", plot_bgcolor: "rgba(0,0,0,0)", font: { family: "JetBrains Mono, monospace", color: "#c9d1de", size: 12 }, xaxis: { title: { text: AXIS_TITLES.x, font: { size: 13, color: "#8e98ac" } }, gridcolor: "rgba(255,255,255,0.05)", zerolinecolor: "rgba(255,255,255,0.08)", linecolor: "#2a3142", ticks: "outside", tickcolor: "#3a4256", mirror: true, showline: true }, yaxis: { title: { text: AXIS_TITLES.y, font: { size: 13, color: "#8e98ac" } }, gridcolor: "rgba(255,255,255,0.05)", zerolinecolor: "rgba(255,255,255,0.08)", linecolor: "#2a3142", ticks: "outside", tickcolor: "#3a4256", mirror: true, showline: true }, legend: { bgcolor: "rgba(17,21,31,0.85)", bordercolor: "#2a3142", borderwidth: 1, font: { color: "#f1f4fb", size: 11 } }, hoverlabel: { bgcolor: "#11151f", bordercolor: "#3a4256", font: { color: "#f1f4fb", family: "JetBrains Mono, monospace" } } },
    light: { paper_bgcolor: "#ffffff", plot_bgcolor: "#ffffff", font: { family: "JetBrains Mono, monospace", color: "#1f2937", size: 12 }, xaxis: { title: { text: AXIS_TITLES.x, font: { size: 13, color: "#4b5563" } }, gridcolor: "rgba(15,23,42,0.09)", zerolinecolor: "rgba(15,23,42,0.18)", linecolor: "#94a3b8", ticks: "outside", tickcolor: "#94a3b8", mirror: true, showline: true }, yaxis: { title: { text: AXIS_TITLES.y, font: { size: 13, color: "#4b5563" } }, gridcolor: "rgba(15,23,42,0.09)", zerolinecolor: "rgba(15,23,42,0.18)", linecolor: "#94a3b8", ticks: "outside", tickcolor: "#94a3b8", mirror: true, showline: true }, legend: { bgcolor: "rgba(255,255,255,0.9)", bordercolor: "#cbd5e1", borderwidth: 1, font: { color: "#111827", size: 11 } }, hoverlabel: { bgcolor: "#ffffff", bordercolor: "#94a3b8", font: { color: "#111827", family: "JetBrains Mono, monospace" } } },
};
